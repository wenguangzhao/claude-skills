(() => {
  ...
  const CONFIG = {
    hexSize: 40,
    containerBgClass: 'bg-slate-50', // light background
    staticFillClass: 'fill-slate-200', // static polygon fill
  ...
  function createHexagon(size, staticFillClass, hoverFillClass) {
    ...
    polygon.addEventListener('mouseenter', () => {
      polygon.classList.remove(staticFillClass, 'duration-[1500ms]');
      polygon.classList.add(hoverFillClass, 'duration-150');
    });

    polygon.addEventListener('mouseleave', () => {
      polygon.classList.remove(hoverFillClass, 'duration-150');
      polygon.classList.add(staticFillClass, 'duration-[1500ms]');
    });
  ...
  async function init() {
    ...
    try { await ensureTailwind(); } catch (e) { /* continue without Tailwind */ }
